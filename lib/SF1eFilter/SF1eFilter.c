...
void SF1eFilterTest()
{
	...
	printf("#SRC SF1eFilter.c\n#CFG {'beta': %f, 'freq': %f, 'dcutoff': %f, 'mincutoff': %f}\n#LOG timestamp, signal, noisy, filtered\n", filter.config.cutoffSlope, filter.config.frequency, filter.config.derivativeCutoffFrequency, filter.config.minCutoffFrequency);
	
	float duration = 10.f; // seconds
    float t = 0.f;
	for(t = 0.f; t < duration; t += 1.f / filter.config.frequency) {
		float signal = sinf(t);
	...
	// if SF1eFilterCreate was used before, make sure do SF1eFilterDestroy(filter) to avoid memory leaks.
}
